...
For now, we'll just start with exporting an empty object.
```js
export default {};
````
...
Themes are a predefined set of styled components used by the component itself.
...
### Update the Theme Map
To add our theme to the theme map, import the `html` theme and add it to the map like the previous themes. Since our copy is named `html` as well, it will override the defaults, thus applying our paragraph styles.
