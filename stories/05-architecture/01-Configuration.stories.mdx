...
This config field expects an array of objects, in which each object has two keys: `key` and `rehydrationFunction`. The key can be in any format acceptable by lodash's [get (or set) functions](https://lodash.com/docs/4.17.15#get). The `rehydrationFunction` will be run to rehydrate data on the client side, if necessary.
```javascript
export default {
