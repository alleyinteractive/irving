...
# Component Mapping
In your Irving application, the `componentMap.js` file is glue between the Components Endpoint and frontend application. This file decides which React components are rendered for any given component name.

## Example
Let's say you want to create a new `my-project/button` component and map it to `irving/button`. You'll need to register and call that component in your template json.
If you want to render an `my-project/button` component,

In your Components Endpoint,
```json
{
  "name": "my-project/button",
  "config": {
    "content": "Hello World"
  }
}
...
You'd need to create the React com
...
import Button from 'components/bogo';

export default {
  ...wordpressComponentMap,
  ...createThemedComponentMap({}),
  'my-project/logo': Logo,
  'my-project/button': Button,
};
```

When the Component Endpoint is parsed by Irving, the JSON structure
...
The root of your Irving application will contain `componentMap.js`
