...
Due to the complexity of implementing [redirections in HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP/Redirections) that work correctly for both client and server-side renders (as well [ proxies](?path=/docs/features-proxies--page) and other application settings) the [Components Endpoint](?path=/docs/components-components-endpoint--page) supports the top-level keys, `redirectTo` and `redirectStatus`.
