...
## Overview
An single instance of Irving is capable (since version 5.2.0) of serving multiple different URLs. Irving will query a different backend API root URL conditionally based on the incoming hostname in the HTTP request. Each backend host should be configured to be compatible with the same component map.
...
When Irving is transpiled, only the `.env` file variables are available in the `process.env` object. Multisite support uses a wrapper object around the base environment variables. The overridden variables are only available through this method, which also requirs the `window.location.href` global to be set.
