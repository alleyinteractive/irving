...
_Note: The information below is older documentation. Checkout the [Example Package](https://github.com/alleyinteractive/irving/tree/master/examples/package) to help speed up this process. Eventually you will be able to scaffold new packages with the CLI._
...
* **`jest.config.js`**: Configuration for Jest tests. This can usually just extend the [base jest configuration](https://github.com/alleyinteractive/irving/blob/master/jest.config.base.js) and modify only the necessary fields. Example:
```javascript
