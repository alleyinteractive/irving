...
### Notes
* `irving/post-meta` sets `single` to true by default, counter to `get_post_meta`, where `$single = false`. This deviation from WordPresss core is to align with how get_post_meta is normally used, vs. maintaining backwards compatibility.
