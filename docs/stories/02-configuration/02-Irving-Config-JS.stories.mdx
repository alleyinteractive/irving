...
If you are replacing the cache service, in order to properly integrate with Irving core, the cache service should return an object that looks like the following:
...
## Disable SSR
* **where to configure**: `irving.config.js` in the `disableSSR` key
* **type**: array
* **purpose**: Disable server-side rendering, forcing Irving to render client-side only. This will switch the client-side script from using `hydrate` to using `render`.
